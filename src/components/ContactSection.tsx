import React, { useState } from 'react';
import { Mail, Send, CheckCircle } from 'lucide-react';
import './ContactSection.css';

export const ContactSection: React.FC = () => {
  const [name, setName] = useState('');
  const [email, setEmail] = useState('');
  const [message, setMessage] = useState('');
  const [submitted, setSubmitted] = useState(false);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !email || !message) return;
    setSubmitted(true);
  };

  return (
    <section className="contact-section" id="contact">
      <div className="contact-card-grid">
        {/* Left Side: Info Panel */}
        <div className="contact-info-panel">
          <div className="info-top">
            <span className="contact-tag">GET IN TOUCH</span>
            <h2 className="contact-display-title">
              LET'S WORK <br />
              <span className="highlight-text">TOGETHER</span>
            </h2>
            <p className="contact-info-desc">
              I am always open to discussing machine learning architectures, interactive generative interfaces, and design systems. Drop me a message or reach out directly.
            </p>
          </div>
          
          <div className="info-bottom">
            <div className="contact-details-list">
              <a href="mailto:hello@dipusardar.com" className="contact-detail-item mail-link">
                <Mail size={18} className="detail-icon" />
                <span>hello@dipusardar.com</span>
              </a>
              <div className="contact-detail-item">
                <span className="detail-label">LOCATION:</span>
                <span>Dhaka, Bangladesh</span>
              </div>
            </div>

            <div className="contact-social-icons">
              <a 
                href="https://github.com/dipusardar" 
                target="_blank" 
                rel="noopener noreferrer" 
                className="social-icon-btn" 
                aria-label="GitHub"
              >
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ width: '20px', height: '20px' }}>
                  <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
                  <path d="M9 18c-4.51 2-5-2-7-2" />
                </svg>
              </a>
              <a 
                href="https://linkedin.com/in/dipusardar" 
                target="_blank" 
                rel="noopener noreferrer" 
                className="social-icon-btn" 
                aria-label="LinkedIn"
              >
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ width: '20px', height: '20px' }}>
                  <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
                  <rect width="4" height="12" x="2" y="9" rx="1" />
                  <circle cx="4" cy="4" r="2" />
                </svg>
              </a>
            </div>
          </div>
        </div>

        {/* Right Side: Form Panel */}
        <div className="contact-form-panel">
          {!submitted ? (
            <form onSubmit={handleSubmit} className="contact-inline-form">
              <div className="inline-form-group">
                <input 
                  type="text" 
                  placeholder="YOUR NAME" 
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  required 
                  className="inline-form-input"
                />
              </div>
              <div className="inline-form-group">
                <input 
                  type="email" 
                  placeholder="YOUR EMAIL" 
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  required 
                  className="inline-form-input"
                />
              </div>
              <div className="inline-form-group">
                <textarea 
                  placeholder="YOUR MESSAGE" 
                  rows={4}
                  value={message}
                  onChange={(e) => setMessage(e.target.value)}
                  required 
                  className="inline-form-textarea"
                ></textarea>
              </div>
              <button type="submit" className="contact-inline-submit">
                <span>SEND MESSAGE</span>
                <Send size={14} className="send-icon" />
              </button>
            </form>
          ) : (
            <div className="contact-inline-success fade-in">
              <CheckCircle size={44} className="success-icon-check" />
              <h3 className="success-title">MESSAGE SENT</h3>
              <p className="success-desc">Thank you for reaching out. I'll get back to you as soon as possible.</p>
              <button onClick={() => setSubmitted(false)} className="success-reset-btn">
                SEND ANOTHER MESSAGE
              </button>
            </div>
          )}
        </div>
      </div>
    </section>
  );
};
